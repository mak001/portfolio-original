...
class ProjectFramework extends DataObject {
    ...
    public function onAfterWrite(){
        ...
        $content = '';
        foreach (ProjectFramework::get() as $framework) {
            $field = DBField::create_field('Color', $framework->BGColor);
            ...
            $content .= "." . $framework->CssClass() . '{ background-color:#' . $color . '; border-color:#' . $color . '; ' . $textColor . '}';
            $content .= "." . $framework->CssClass() . ':hover{ background-color:#' . $hover . '; border-color:#' . $border . '; ' . $hoverColor. '}';
        }
        
        $fh = fopen(ASSETS_PATH . '/css/frameworks.css', 'w');
        fwrite($fh, $content);
        fclose($fh);
    }
